...
export default function Page() {
  ...
  useEffect(() => {
    fetch('/api/admin/settings?section=auth').then(r => r.json()).then(d => setData(d)).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Paramètres authentification</h1>
        <p className="text-muted-foreground mt-1">Configuration: Paramètres authentification</p>
      </div>

...
// Zones manuelles pour régions sans GeoJSON
const MANUAL_ZONES: { name: string; region: string; coords: {lat:number;lng:number}[] }[] = [
  // LAVAL
...
interface Driver {
  id: string; name: string; lat: number; lng: number; status: string; zone: string;
}
...
export default function LiveMapPage() {
  ...
  // Charger le GeoJSON Montréal depuis /public
  useEffect(() => {
    fetch("/montreal_zones.json")
      .then(r => r.json())
      .then(d => { geoDataRef.current = d; setGeoLoaded(true); })
      .catch(() => { setGeoLoaded(true); }); // continue même sans GeoJSON
  }, []);

  // Init carte quand Maps + GeoJSON prêts
  useEffect(() => {
  ...
  // Redessiner quand filtres changent
  useEffect(() => {
  ...
  function drawAll(active: Set<string>) {
    ...
    // 1. GeoJSON réel (Montréal + West Island + Sud-Ouest depuis fichier)
    if (geoDataRef.current) {
      geoDataRef.current.features.forEach((feat: any) => {
        ...
        rings.forEach((ring: any) => {
          const paths = ring.map((loop: any) =>
            loop.map(([lng, lat]: [number, number]) => ({ lat, lng }))
          );
          ...
          poly.addListener("click", (e: any) => {
            infoWindow.setContent(`<div style="font-family:sans-serif;padding:6px 12px"><b style="font-size:14px">${name}</b><br/><span style="font-size:12px;color:#6b7280">${s.label}</span></div>`);
            infoWindow.setPosition(e.latLng);
            infoWindow.open(mapInst.current);
          });
    ...
    MANUAL_ZONES.forEach(zone => {
      ...
      poly.addListener("click", (e: any) => {
        infoWindow.setContent(`<div style="font-family:sans-serif;padding:6px 12px"><b style="font-size:14px">${zone.name}</b><br/><span style="font-size:12px;color:#6b7280">${s.label}</span></div>`);
        infoWindow.setPosition(e.latLng);
        infoWindow.open(mapInst.current);
      });
      polygons.current.push(poly);
  ...
  const totalZones = (geoDataRef.current?.features?.filter((f:any) => activeRegions.has(f.properties.region)).length || 0)
    + MANUAL_ZONES.filter(z => activeRegions.has(z.region)).length;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label:"Zones affichées", value:totalZones, color:"text-blue-600", bg:"bg-blue-50 border-blue-200" },

...
export default function LiveMapPage() {
  ...
  // 2. Charger GeoJSON depuis /public
  useEffect(() => {
  ...
  // 3. Init carte quand tout est prêt
  useEffect(() => {
  ...
  function drawPolygons(active: Set<string>) {
    ...
    geoData.current.features.forEach((feat: any) => {
      ...
      allRings.forEach((ring: number[][][]) => {
        // ring[0] = outer boundary, ring[1+] = holes
  ...
  function toggleRegion(region: string) {
    setActiveRegions(prev => { const n=new Set(prev); n.has(region)?n.delete(region):n.add(region); return n; });
  }

...
export default function DriversDocumentsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/drivers/documents")
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Documents chauffeurs</h1>
        <p className="text-muted-foreground mt-1">Vérification des documents soumis par les chauffeurs</p>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Type document</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Statut</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Expiration</th>
                </tr>
              </thead>

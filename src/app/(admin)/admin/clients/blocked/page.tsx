...
export default function ClientsBlockedPage() {
  const [items, setItems] = useState<Record<string, unknown>[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");

  useEffect(() => {
    fetch("/api/admin/clients?status=blocked")
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Clients bloqués</h1>
        <p className="text-muted-foreground mt-1">Clients suspendus ou bannis de la plateforme</p>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Date blocage</th>
                </tr>
              </thead>

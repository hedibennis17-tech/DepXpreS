...
export default function ClientPromotionsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/clients/promotions")
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Promotions utilisées</h1>
        <p className="text-muted-foreground mt-1">Codes promo et réductions utilisés par ce client</p>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Réduction</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Date utilisation</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Commande</th>
                </tr>
              </thead>

...
export default function ZonesInactivePage() {
  const [items, setItems] = useState<Record<string, unknown>[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");

  useEffect(() => {
    fetch("/api/admin/zones?status=inactive")
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Zones de livraison désactivées</p>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase">Date désactivation</th>
                </tr>
              </thead>
